...
public class MyHolidayLights extends JFrame{
	...
	public MyHolidayLights(HolidayLights hl) {
	...
	private static class LightWindow extends JComponent {
		...
		public LightWindow(HolidayLights hl) {
			this.hl = hl;
			this.timer = new Timer(MyHolidayLights.millsPerFrame, new ActionListener(){
				public void actionPerformed(ActionEvent e) {
					update();
				}
			});
		...
		@Override public void paintComponent(Graphics g) {
			...
			ColoredLight color=new ColoredLight();
			int curX = 10;
			int curY = 30;
			...
			for (int i = 0; i < this.lightState.size(); i++) {
				...
				if (l.isOn()) {
					g2d.setColor(color.randomColor());
